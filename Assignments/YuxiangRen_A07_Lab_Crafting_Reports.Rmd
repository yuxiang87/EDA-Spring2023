...
```{r project.setup, message=FALSE, error=FALSE, echo=TRUE}
#Import libraries
...
```{r boxplots 2018, echo=FALSE, fig.align='left',fig.height=3,warning=FALSE,fig.cap= "2018 Ozone levels by month"}
Ozone_2018 <- filter(epa_data, year(Date)==2018)
ggplot(Ozone_2018, aes(x = factor(month(Date), level = 1:12, labels = c(month.abb)), y=Ozone)) + 
  geom_boxplot()+
  labs(x = "Month", y = "Ozone")

```

```{r boxplots 2019, echo=FALSE, fig.align='left',fig.height=3,warning=FALSE, fig.cap= "2019 Ozone levels by month"}
Ozone_2019 <- filter(epa_data, year(Date)==2019)
ggplot(Ozone_2019, aes(x = factor(month(Date), level = 1:12, labels = c(month.abb)), y=Ozone)) + 
  geom_boxplot()+
  labs(x = "Month", y = "Ozone")
```
...
### Git Repository
https://github.com/yuxiang87/EDA-Spring2023.git
