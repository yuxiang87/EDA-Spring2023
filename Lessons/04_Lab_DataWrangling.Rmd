...
NTL.phys.19991990.Tues <- NTL.phys.data %>% filter(lakename == "Tuesday Lake") %>% 
  filter(year4 %in% c(1990:1999))


summary(NTL.phys.19991990.Tues)
```
...
NTL.phys.19991990.Tues.July <- NTL.phys.data %>% 
  filter(lakename == "Tuesday Lake") %>% 
  filter(year4 %in% c(1990:1999)) %>% 
  filter(daynum %in% c(182:210)) 
summaries(NTL.phys.19991990.Tues.July$sampledate)  
...
is.na(NTL.phys.19991990.Tues.July$temperature_C)
NTL.phys.19991990.Tues.July.meanT <- NTL.phys.19991990.Tues.July %>% 
  filter(depth == 0) %>% 
  filter(!is.na(temperature_C)) %>% 
  summarise(meanTemperature = mean(temperature_C))

# or use drio_na
NTL.phys.19991990.Tues.July.meanT_2 <- NTL.phys.19991990.Tues.July %>% 
  filter(depth == 0) %>% 
  drop_na(temperature_C) %>% 
  summarise(meanTemperature = mean(temperature_C))
  
summary(NTL.phys.19991990.Tues.meantemperature)
is.na(NTL.phys.19991990.Tues.meantemperature$depth)
...
NTL.Irradiance <- pivot_longer(NTL.phys.data,irradianceWater:irradianceWater, names_to = "Irradiance_Type",values_to = "Irradiance_Value")

#Exercise 6: Spread temperatureC into more than one column based on the depth.
NTL.depthclass.temperatureC.spread <-NTL.phys.data %>% 
