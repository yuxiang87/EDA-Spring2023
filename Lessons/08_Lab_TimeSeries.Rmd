...
##Note-using time sequence to left join the oringinal data.
summary(BR_complete$Date)
summary(BR_complete)
...
#Rainfall
Rainfall_Plot <- 
ggplot(BR_complete, aes(Date, Rainfall))+
  geom_line()+
...
#Temperature
Temperature_C <- 
ggplot(BR_complete, aes(Date, Temperature_C))+
  geom_line()+
  geom_smooth(method = "lm")
plot(Temperature_C)
...
ts_temperature <- ts(BR_complete$Temperature_C, start = c(f_year,f_month), frequency = 12)

ts_Rainfall <- ts(BR_complete$Rainfall, start = c(1901,1), frequency = 12)
...
stl_ts_temperature <- stl(ts_temperature, s.window = "periodic")
plot(stl_ts_temperature)

stl_ts_Rainfall <- stl(ts_Rainfall, s.window = "periodic")
plot(stl_ts_Rainfall)
...
##temperature
Temperature_Components <- as.data.frame(stl_ts_temperature$time.series)
Temperature_Components <- mutate(Temperature_Components,
          Observed = BR_complete$Temperature_C,     
        Date = BR_complete$Date)
##Rainfall
Rainfall_Components <- as.data.frame(stl_ts_Rainfall$time.series)
Rainfall_Components <- mutate(Rainfall_Components,
          Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date,seasonal_shift = Rainfall_Components$seasonal + 145.1604)
mean(Rainfall_Components$trend)
...
ggplot(Rainfall_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = seasonal_shift, x = Date), color = "#c13d75ff") 
```
...
Temp_trend <- trend::smk.test(ts_temperature)
