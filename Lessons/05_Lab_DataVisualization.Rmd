...
str(PeterPaul.chem.nutrients$month)
...
ggplot(PeterPaul.chem.nutrients, aes(x = factor(month, level = 1:12, labels = c(month.abb))))+
  geom_bar()+
...
# 1.
surface <- filter(PeterPaul.chem.nutrients, depth == "0")
ggplot(surface, aes(x = daynum, y = temperature_C, color = year4))+
         geom_point(size= 0.5)+
  facet_wrap(vars(lakename), nrow = 2)+
  ylab('Suurface Temperature')

PeterPaul.chem.nutrients %>% filter(depth == 0) %>% 
