...
huc8_sf_2 <- st_read('./Data/Spatial/NCHUC8.shp') %>% 
  filter(SUBBASIN == "Upper Neuse")


#Create a map
mapview(huc8_sf, col.regions = 'orange')+ 
  mapview(huc8_sf_2, col.regions = 'purple')
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% #note fips is factor
...
huc8_sf_merge = merge(x = huc8_sf,
                     y = wind_raw,
                     by.x = 'HUC_8',
                     by.y = 'HUC_8')
huc8_sf_join <- huc8_sf %>% left_join(wind_raw, by = c('HUC_8'))


#View the outputs
mapview(huc8_sf_merge,
        zcol = 'SumWindEnergy')
mapview(huc8_sf_join,
        zcol = 'SumWindEnergy')
...
unique(huc8_sf_join$DWQ_Basin)


#Summarize on DWQ Basin value
huc2_sf <- huc8_sf_join %>% group_by(DWQ_Basin) %>% summarize(ACRES = sum(ACRES), SQ_MILES = sum(SQ_MILES))
...
myMap + mapview(neuse_intersect_3, alpha.regions = 0)
...
Triangle_sf <- counties_sf_utm %>% filter(NAME %in% c("Chatham", "Durham","Orange","Wake"))
mapview(Triangle_sf)
mapview(counties_sf_utm)


#Grab the intersecting HUC_8s
Tringle_HUC8_sf <- huc8_sf_utm %>% 
  st_intersection(Triangle_sf)
myMap + mapview(Tringle_HUC8_sf, alpha.regions = 0)


#Intersect the HUC_8s
Tringle_HUC8_sf <- Tringle_HUC8_sf %>% 
  mutate(Area_m2 = as.numeric(st_area(Tringle_HUC8_sf$geometry)))
mapview(Tringle_HUC8_sf)
